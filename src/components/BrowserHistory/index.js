...
class BrowserHistory extends Component {
  ...
  deleteHistory = id => {
    ...
    const updatedHistoryList = historyListItems.filter(
      eachHistory => eachHistory.id !== id,
    )

    this.setState({
      historyListItems: updatedHistoryList,
    })
  }
  ...
  render() {
    ...
    const filterHistoryItems = historyListItems.filter(eachItem =>
        ...
        <div>
          {filterHistoryItems.length > 0 ? (
            <ul className="unordered-list-Container">
              {filterHistoryItems.map(eachItem => (
                <ListItem
                  listDetails={eachItem}
                  key={eachItem.id}
                  deleteHistory={this.onListDelete}
                />
              ))}
            </ul>
          ) : (
